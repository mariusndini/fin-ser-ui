...
function getAsssetSparkPerfSpark(asset){
    var SQL = ` SELECT *
                ...
                WHERE TICKER_REGION ='`+asset+`-US'
                      and date between current_date() - 3650 and current_date()
                order by date asc;`
    make_esg(asset);
    runSQL(gbl.indconn, SQL).then((res)=>{
        var rev = []
...
function make_esg(asset){
    var SQL = ` SELECT *
                FROM FIN_DEMO_UI.META.TICKER_ESG
                WHERE TICKER_REGION ='`+asset+`-US'
                order by d asc;`
    ...
    runSQL(gbl.indconn, SQL).then((res)=>{
        ...
        for(i=0; i < res.length; i++){
            esg.push(res[i].PCT)
            if(i == res.length-1){
                $('#esg-spark').html ('ESG <b style="font-size:10px;">(' +res[i].RANK+ ')</b>');
            }
        }//end big for loop

        $("#asset-table-esg-spark").sparkline(esg, {
            type: 'line',
            width: '150px',
            height: '50px',
            lineColor: "#FFF",
